  ...
  try {
    ...
    if (view === 'version-news') {
      if (req.method === 'GET') {
        ...
        applyCors(res);
        res.setHeader('Cache-Control', 'public, s-maxage=120, stale-while-revalidate=600, max-age=30');
    ...
    if (req.method === 'GET') {
      ...
      applyCors(res);
      res.setHeader('Cache-Control', 'public, s-maxage=60, stale-while-revalidate=300, max-age=15');
